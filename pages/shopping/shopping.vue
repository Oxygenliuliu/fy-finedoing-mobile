	<template>
		...
		<view  :class="(pageIsShow == false || net == false) == true?'container':''" class="tocuh-header">
			<view v-if="pageIsShow == true && net == true">
				<!-- #ifdef H5 -->
					<view class="topNum" v-if="shopNums!=0">({{shopNums}})</view>
				<!-- #endif -->
				<view :class="{actives:escActive}" class="shop-list">
					<view class="shop-space"></view>
					<view class="uni-shopinfo" v-for="(item, index) in shoppingList" :key="index">
						<view class="uni-middle flex-row">
							<view class="flex-view-item flex-left">
								<label class="checkbox bottomLable"  @tap="checkboxChange(index)">
									<radio value="" :checked="(item.chec == 1?true:false)" borradius="30px" style="transform: scale(0.9)" color="red" />
								</label>
							</view>
							<view class="flex-view-item flex-center">
							...
							<view class="flex-column" style="display:flex;flex-direction: column;" >
									<view class="flex-view-item titleName" style="padding-left: 10px;">
										<text>{{item.ty_name}}</text>
									</view>
									...
									<view class="flex-view-item" style="display: flex;flex-direction: row;padding-left: 10px;">
										...
										<view class="flex-view-item InpBtn">
											<button :data-scid="(item.sc_id)" @click="reduceBtn(index)" class="plus" :class="{disabled:(item.sc_number <= 1 ? true : false)}">-</button>
											<input type="number" class="inputNum" v-model="item.sc_number" :data-scid="(item.sc_id)" :data-index="index" @blur="numberName">
											<button :data-scid="(item.sc_id)" @click="plusBtn(index)" class="plus">+</button>		
										</view>
									</view>
								</view>
							</view>
					...
					<view class="uni-bottoms">
						...
						<view class="btnSF">
							<view :class="{actives:btnDelete}" @tap="publicBtn(1)" v-if="deleteId.length>0">删 除({{sltdGoods}})</view>
							<!-- <view @tap="publicBtn(1)" v-if="deleteId.length>0">删 除({{sltdGoods}})</view> -->
						</view>
						<view class="morend">
							<view style="font-size: 14px;">总计:<span>￥ {{TotalAmount}}</span></view>
						</view> 
						<view class="ends">
							<view @tap="publicBtn(2)">去结算({{sltdGoods}})</view>
						</view>
					</view>
				</view>
					
				<!-- 规格-模态层弹窗 组件 -->
				<uni-makings ref="unimakings" @change="makchange" @changeInfo="retPrice"></uni-makings>
				<!-- 当购物车没有商品时显示的板块 -->
					...
					<view>
						<view>
							...
							<view class="goods-list">
								<view class="flex-row goods-list-flex">
			...
			<view class="noData" v-if="pageIsShow == false || net == false">
				<noData :net="net" @handle='StatusMsg'></noData>
			</view>
			<!-- #ifdef H5 -->
			<tarBar :active='2' ref='child'></tarBar>
			<!-- #endif -->
		</view>
</template>
<script>
	...
	export default {
		components: {uniIcon,uniNumberBox,tarBar,uniMakings,noData},
		data() {
			return {
				net:true,// 没有网络
				...
				e:0, //下标
				unExist:'', //默认替换图片
				buyOrShopcart:3,//0代表都没点，1.购物车，2.直接购买,3.购物车页面发起
		...
		methods:{
			...
			upNumber(IndexNum){ //修改商品信息调用的接口
				uni.request({
					url:getApp().globalData.webUrl+'/mobile/index/upShopCartData',
					method:'POST',
					data:{
						Ident_Signboard: this.Signboard,
						Ident_Signguid: this.Signguid,
						sid: this.shoppingList[IndexNum].sc_id, //购物车序号
						gid: this.shoppingList[IndexNum].ty_id, //商品序号
						clid: this.shoppingList[IndexNum].g_id, //材料序号
						qty: this.shoppingList[IndexNum].sc_number, //商品数量
						total: this.getTotal, //小计
						price: this.temp.total, //单价
						lpid: this.shoppingList[IndexNum].lpid, //工艺序号
						lpname: this.shoppingList[IndexNum].lpname, //工艺名称
			...
			makchange(res){
				...
				this.shoppingList[this.lineindex].lpid = '';
				...
				this.getTotal = res.totalPrice;  //总价
				this.shoppingList[this.lineindex].total = res.totalPrice;  //价格
				...
				this.upNumber(this.lineindex); //调用商品信息修改接口
			},
			// 点击材料切换选中
			NameTap(index,e){
				this.isActive =true;
				...
				uni.request({
					...
					success: (res) => {
						...
						this.gongYiId=this.shoppingList[index].lpid.split(","); //工艺Id
						// 请求参数数组：下单页
						let shopArray={"Signboard":this.Signboard,"Signguid":this.Signguid,"Price":this.temp.total,"stuffList":this.makings,"unitlist":this.unitlist,"shopImgs":this.temp.img,"Partnerid":this.Partnerid,"dbname":this.dbname,"token":this.token,"org":this.org,"userName":this.uName,'rSelect':this.gongYiName,'gongYiId':this.gongYiId,'caiLInfo':this.shoppingList[index].g_name,'caiLiaoId':this.shoppingList[index].g_id,'guigWidth':splitSize[0],'guigHeight':splitSize[1],'type_id':this.shoppingList[index].ty_id,'number':this.shoppingList[index].sc_number,'buyOrShopcart':this.buyOrShopcart}
						//调用规格弹出框
			...
			operapeShop(e){//小程序这个函数调用不起
				...
				uni.request({
					url: getApp().globalData.webUrl+'/mobile/index/OperateShop',
					method: 'POST',
					data: {
					...
					success: (res) => {
						...
						if(res.data.status != 0){
							uni.showToast({title:'信息修改失败',icon: 'none'})
						}else{
						 }
					},
					fail:(e)=>{
			...
			numberName(e){
				...
				if((/^[0]+/).test(e.target.value.toString()) || e.target.value==''||e.target.value==null ||e.target.value==0 ||e.target.value.toString().indexOf('.') != -1 || (/^[0]+/).test(e.target.value) ){
					...
					// 	return false;
					// }else{
						// this.shoppingList[this.lineindex].sc_number = this.shoppingList[this.lineindex].sc_number.toString().replace(/^0+/,"")
						// //this.shoppingList[this.lineindex].sc_number = parseInt(e.target.value);  //改变数量时：给它重新赋值
						if(this.shoppingList[this.lineindex].sc_number.length >=10 ){
							uni.showToast({
								title: '超出最大数量',
								icon: 'none'
							});
			...
			// 去结算按钮
			Settlement(e){
				if(uni.getStorageSync("publicbtn") != 'click'){
					var wulala=uni.getStorageSync("jsonList");  //获取存储在Storage里的值
					var data = JSON.parse(wulala); //JSON字符串转对象
					var Signboard = data.Ident_Signboard;  //给Signboard 赋值最近更新
					var Signguid = data.Ident_Signguid;  //给Signguid 赋值
					...
					uni.setStorageSync('orderCreation',this.gId)
				...
				uni.showModal({  //模态弹窗
					...
					success: (res)=> {
						if (res.confirm) {
							...
							uni.request({
								url: getApp().globalData.webUrl+'/mobile/Classification/removeShop',
								method: 'POST',
								data: {
									Ident_Signboard: Signboard,
									Ident_Signguid: Signguid,
								...
								success: (res) => {
									...
									if(res.data.status == 0){
										...
										uni.removeStorageSync('publicbtn')
			...
			publicBtn(e){
				if(e == 1){
					uni.setStorageSync('publicbtn','click');
					this.deleteCart(e);
				}else{
					this.Settlement(e);
				}
			...
			hotGood(){ //获取热销商品
				//判断csid是否为空
				uni.request({
					url:getApp().globalData.webUrl+'/mobile/classification/getHotGood',
					method:'GET',
					data:{},
					success: (res) => {
						if(res.data.status==0){
			...
			getShopList(){ //获取购物车商品信息
				...
				uni.request({
					url: getApp().globalData.webUrl+'/mobile/index/getShopping',
					method:'POST',
					data:{
						Ident_Signboard:this.Signboard,
						Ident_Signguid: this.Signguid
					...
					try{
						...
						if(res.data.status == 0){
							...
							uni.setStorageSync('jsonList',json);
								that.shoppingList=[];
								that.currentPage=1;
								res.data.shopping.forEach(item=> {
									item.sc_number += '';
								})
			...
			provider(){ //分页
				...
				for(that.e;that.e < num; that.e++){
					...
					if(that.shopPaging[that.e].lpname instanceof Array){
						
					}else{
						that.shopPaging[that.e].lpname = that.shopPaging[that.e].lpname.replace(/["\[\]]/g,'');
					}
